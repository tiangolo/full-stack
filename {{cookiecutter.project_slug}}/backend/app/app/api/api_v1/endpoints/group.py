...
@docs.register
@doc(description="Create a new group", security=security_params, tags=["groups"])
...
@marshal_with(GroupSchema(only=("id", "name", "created_at", "name"), many=True))
@jwt_required
...
@docs.register
@doc(
    description="Assign user as group Admin", security=security_params, tags=["groups"]
)
@app.route(f"{config.API_V1_STR}/groups/<int:group_id>/admin_users/", methods=["POST"])
